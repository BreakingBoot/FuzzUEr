    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%5)
        {
            case 0:
                ...
                break;
            case 1:
                Status = FuzzStatusCheck(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzHashAll(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzLogEvent(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzPassThroughToTpm(&Input, SystemTable, ImageHandle);
                break;
        }
    }
