...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzExecuteScsiCommand(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzGetDeviceType(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzResetDevice(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzResetBus(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzGetDeviceLocation(&Input, SystemTable, ImageHandle);
                break;
        }
    }
