...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzNewImageEx(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzGetImageEx(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzSetImageEx(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzDrawImageEx(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzDrawImageIdEx(&Input, SystemTable, ImageHandle);
                break;
            case 5:
