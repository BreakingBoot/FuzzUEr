...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzGetNetworkInterfaceList(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzAcquireRedfishService(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzAbortAcquireRedfishService(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzReleaseRedfishService(&Input, SystemTable, ImageHandle);
                break;
        }
    }
