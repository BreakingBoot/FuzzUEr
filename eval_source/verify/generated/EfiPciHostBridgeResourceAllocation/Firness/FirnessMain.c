...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzGetNextRootBridge(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzSubmitResources(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzNotifyPhase(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzGetProposedResources(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzStartBusEnumeration(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzSetBusNumbers(&Input, SystemTable, ImageHandle);
                break;
            case 6:
                Status = FuzzGetAllocAttributes(&Input, SystemTable, ImageHandle);
                break;
            case 7:
                Status = FuzzPreprocessController(&Input, SystemTable, ImageHandle);
                break;
        }
    }
