...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%4)
        {
            case 0:
                Status = FuzzGetMaximumProcessorIndex(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzRegisterPeriodicCallback(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzRegisterExceptionCallback(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzInvalidateInstructionCache(&Input, SystemTable, ImageHandle);
                break;
        }
    }
