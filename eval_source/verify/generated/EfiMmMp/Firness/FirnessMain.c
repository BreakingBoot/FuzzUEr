    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%6)
        {
            case 0:
                ...
                break;
            case 1:
                Status = FuzzDispatchProcedure(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzBroadcastProcedure(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzSetStartupProcedure(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzWaitForProcedure(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzCheckForProcedure(&Input, SystemTable, ImageHandle);
                break;
        }
    }
