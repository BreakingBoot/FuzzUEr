...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzSetSessionData(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzBuildResponsePacket(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzGetSessionData(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzProcessPacket(&Input, SystemTable, ImageHandle);
                break;
        }
    }
