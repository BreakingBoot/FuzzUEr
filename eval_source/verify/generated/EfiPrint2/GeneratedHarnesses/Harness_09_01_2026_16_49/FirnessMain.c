...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%6)
        {
            case 0:
                Status = FuzzUnicodeBSPrint(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzUnicodeBSPrintAsciiFormat(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzUnicodeValueToString(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzAsciiBSPrint(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzAsciiBSPrintUnicodeFormat(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzAsciiValueToString(&Input, SystemTable, ImageHandle);
                break;
        }
    }
