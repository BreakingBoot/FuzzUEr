...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzSubmitData(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzCalculateMode(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzSetTiming(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzNotifyPhase(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzGetChannelInfo(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzDisqualifyMode(&Input, SystemTable, ImageHandle);
                break;
        }
    }
