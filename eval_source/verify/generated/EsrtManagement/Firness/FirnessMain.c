...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzGetEsrtEntry(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzUpdateEsrtEntry(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzRegisterEsrtEntry(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzUnRegisterEsrtEntry(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzSyncEsrtFmp(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzLockEsrtRepository(&Input, SystemTable, ImageHandle);
                break;
        }
    }
