...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzMemoryTestInit(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzPerformMemoryTest(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzFinished(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzCompatibleRangeTest(&Input, SystemTable, ImageHandle);
                break;
        }
    }
