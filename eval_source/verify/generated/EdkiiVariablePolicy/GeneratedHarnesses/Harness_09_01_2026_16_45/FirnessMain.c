...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%5)
        {
            case 0:
                Status = FuzzDisableVariablePolicy(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzRegisterVariablePolicy(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzLockVariablePolicy(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzIsVariablePolicyEnabled(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzDumpVariablePolicy(&Input, SystemTable, ImageHandle);
                break;
        }
    }
