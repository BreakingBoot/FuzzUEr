    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%7)
        {
            case 0:
                ...
                break;
            case 1:
                Status = FuzzResetDevice(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzBuildDevicePath(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzGetDevice(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzGetNextPort(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzGetNextDevice(&Input, SystemTable, ImageHandle);
                break;
            case 6:
                Status = FuzzResetPort(&Input, SystemTable, ImageHandle);
                break;
        }
    }
