...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzGetPciRom(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzGetPlatformPolicy(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzPlatformNotify(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzPlatformPrepController(&Input, SystemTable, ImageHandle);
                break;
        }
    }
