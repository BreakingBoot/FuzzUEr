...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzPossibleResources(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzSetResources(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzRegisterAccess(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzModify(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzGetResources(&Input, SystemTable, ImageHandle);
                break;
        }
    }
