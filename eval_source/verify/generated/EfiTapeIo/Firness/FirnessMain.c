...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzTapeRead(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzTapeWrite(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzTapeRewind(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzTapeSpace(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzTapeWriteFM(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzTapeReset(&Input, SystemTable, ImageHandle);
                break;
        }
    }
