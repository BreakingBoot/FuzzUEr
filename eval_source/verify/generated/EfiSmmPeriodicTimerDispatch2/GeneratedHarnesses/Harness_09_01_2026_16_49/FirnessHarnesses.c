...
FuzzRegister(
    ...
    EFI_SMM_PERIODIC_TIMER_DISPATCH2_PROTOCOL * ProtocolVariable = NULL;
    Status = SystemTable->BootServices->LocateProtocol(&gEfiSmmPeriodicTimerDispatch2ProtocolGuid, NULL, (VOID *)&ProtocolVariable);
    if (EFI_ERROR(Status)) {
        return Status;
    }
    /*
        Input Variable(s)
    */
    EFI_SMM_HANDLER_ENTRY_POINT2 Register_Arg_1 = 0;
    EFI_SMM_PERIODIC_TIMER_REGISTER_CONTEXT * Register_Arg_2 = (EFI_SMM_PERIODIC_TIMER_REGISTER_CONTEXT *)AllocateZeroPool(sizeof(EFI_SMM_PERIODIC_TIMER_REGISTER_CONTEXT));
    ...
    {
        UINT64 Firness_SmiTickInterval;
        ReadBytes(Input, sizeof(Firness_SmiTickInterval), (VOID *)&Firness_SmiTickInterval);
        Register_Arg_2->SmiTickInterval = Firness_SmiTickInterval;
    }
