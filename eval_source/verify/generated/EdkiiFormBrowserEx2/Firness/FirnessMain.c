...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzIsBrowserDataModified(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzExecuteAction(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzIsResetRequired(&Input, SystemTable, ImageHandle);
                break;
        }
    }
