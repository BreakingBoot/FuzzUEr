...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzUnloadImage(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzCreateThunk(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzGetVersion(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzRegisterICacheFlush(&Input, SystemTable, ImageHandle);
                break;
        }
    }
