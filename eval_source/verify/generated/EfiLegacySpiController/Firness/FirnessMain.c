...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzEraseBlockOpcode(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzWriteStatusPrefix(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzBiosBaseAddress(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzClearSpiProtect(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzIsRangeProtected(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzProtectNextRange(&Input, SystemTable, ImageHandle);
                break;
            case 6:
                Status = FuzzLockController(&Input, SystemTable, ImageHandle);
                break;
        }
    }
