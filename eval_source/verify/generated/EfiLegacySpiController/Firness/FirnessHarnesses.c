...
/*
    ...
    called EraseBlockOpcode.
*/
__attribute__((no_sanitize("address")))
EFI_STATUS
EFIAPI
FuzzEraseBlockOpcode(
    IN INPUT_BUFFER *Input,
    IN EFI_SYSTEM_TABLE *SystemTable,
    IN EFI_HANDLE *ImageHandle
) {
    EFI_STATUS Status = EFI_SUCCESS;
    EFI_LEGACY_SPI_CONTROLLER_PROTOCOL * ProtocolVariable = NULL;
    Status = SystemTable->BootServices->LocateProtocol(&gEfiLegacySpiControllerProtocolGuid, NULL, (VOID *)&ProtocolVariable);
    if (EFI_ERROR(Status)) {
        return Status;
    }
    /*
        Input Variable(s)
    */
    UINT8 EraseBlockOpcode_Arg_1 = 0;
    
    // Fuzzable Variable Initialization
    ReadBytes(Input, sizeof(EraseBlockOpcode_Arg_1), (VOID *)&EraseBlockOpcode_Arg_1);
    
    Status = ProtocolVariable->EraseBlockOpcode(
        ProtocolVariable,
        EraseBlockOpcode_Arg_1
    );
    return Status;
}
...
/*
    This is a harness for fuzzing the protocol service
    called WriteStatusPrefix.
*/
__attribute__((no_sanitize("address")))
EFI_STATUS
EFIAPI
FuzzWriteStatusPrefix(
    IN INPUT_BUFFER *Input,
    IN EFI_SYSTEM_TABLE *SystemTable,
    IN EFI_HANDLE *ImageHandle
) {
    ...
    /*
        Input Variable(s)
    */
    UINT8 WriteStatusPrefix_Arg_1 = 0;
    
    // Fuzzable Variable Initialization
    ReadBytes(Input, sizeof(WriteStatusPrefix_Arg_1), (VOID *)&WriteStatusPrefix_Arg_1);
    
    Status = ProtocolVariable->WriteStatusPrefix(
        ProtocolVariable,
        WriteStatusPrefix_Arg_1
    );
    return Status;
}
