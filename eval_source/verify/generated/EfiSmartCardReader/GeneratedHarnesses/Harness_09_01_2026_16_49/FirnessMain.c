...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzSCardConnect(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzSCardDisconnect(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzSCardStatus(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzSCardTransmit(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzSCardControl(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzSCardGetAttrib(&Input, SystemTable, ImageHandle);
                break;
        }
    }
