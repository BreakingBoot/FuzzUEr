...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%3)
        {
            case 0:
                Status = FuzzRegisterSetVariableCheckHandler(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzVariablePropertySet(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzVariablePropertyGet(&Input, SystemTable, ImageHandle);
                break;
        }
    }
