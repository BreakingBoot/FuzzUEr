...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzStriColl(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzMetaiMatch(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzStrUpr(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzStrLwr(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzFatToStr(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzStrToFat(&Input, SystemTable, ImageHandle);
                break;
        }
    }
