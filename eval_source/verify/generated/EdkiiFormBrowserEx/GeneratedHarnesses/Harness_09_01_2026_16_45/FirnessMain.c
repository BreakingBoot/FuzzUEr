...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzSetScope(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzRegisterHotKey(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzRegiserExitHandler(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzSaveReminder(&Input, SystemTable, ImageHandle);
                break;
        }
    }
