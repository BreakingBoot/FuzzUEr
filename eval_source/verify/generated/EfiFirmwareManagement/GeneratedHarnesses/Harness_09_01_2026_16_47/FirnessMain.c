...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzGetImageInfo(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzGetImage(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzSetImage(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzCheckImage(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzGetPackageInfo(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzSetPackageInfo(&Input, SystemTable, ImageHandle);
                break;
        }
    }
