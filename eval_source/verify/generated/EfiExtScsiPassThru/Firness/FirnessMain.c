...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzGetTargetLun(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzGetNextTargetLun(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzResetChannel(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzResetTargetLun(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzPassThru(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzBuildDevicePath(&Input, SystemTable, ImageHandle);
                break;
            case 6:
                Status = FuzzGetNextTarget(&Input, SystemTable, ImageHandle);
                break;
        }
    }
