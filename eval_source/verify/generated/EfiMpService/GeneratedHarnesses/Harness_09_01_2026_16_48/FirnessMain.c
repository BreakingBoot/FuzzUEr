...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzGetNumberOfProcessors(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzGetProcessorInfo(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzStartupAllAPs(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzStartupThisAP(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzSwitchBSP(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzEnableDisableAP(&Input, SystemTable, ImageHandle);
                break;
            case 6:
                Status = FuzzWhoAmI(&Input, SystemTable, ImageHandle);
                break;
        }
    }
