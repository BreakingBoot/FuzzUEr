...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzBlockToConfig(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzConfigToBlock(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzRouteConfig(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzExtractConfig(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzExportConfig(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzGetAltConfig(&Input, SystemTable, ImageHandle);
                break;
        }
    }
