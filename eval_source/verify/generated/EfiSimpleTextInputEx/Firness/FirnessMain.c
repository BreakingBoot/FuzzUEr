    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%5)
        {
            case 0:
                ...
                break;
            case 1:
                Status = FuzzReadKeyStrokeEx(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzSetState(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzRegisterKeyNotify(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzUnregisterKeyNotify(&Input, SystemTable, ImageHandle);
                break;
        }
    }
