...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        {
            case 0:
                Status = FuzzGetTimerPeriod(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzSetTimerPeriod(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzRegisterHandler(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzGenerateSoftInterrupt(&Input, SystemTable, ImageHandle);
                break;
        }
    }
