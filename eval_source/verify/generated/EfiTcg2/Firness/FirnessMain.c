...
FirnessMain (
    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%7)
        {
            case 0:
                Status = FuzzHashLogExtendEvent(&Input, SystemTable, ImageHandle);
                break;
            case 1:
                Status = FuzzGetCapability(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzGetEventLog(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzSubmitCommand(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzGetActivePcrBanks(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzSetActivePcrBanks(&Input, SystemTable, ImageHandle);
                break;
            case 6:
                Status = FuzzGetResultOfSetActivePcrBanks(&Input, SystemTable, ImageHandle);
                break;
        }
    }
