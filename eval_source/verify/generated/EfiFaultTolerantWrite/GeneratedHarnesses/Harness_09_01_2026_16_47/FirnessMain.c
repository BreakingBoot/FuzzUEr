    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%6)
        {
            case 0:
                ...
                break;
            case 1:
                Status = FuzzGetMaxBlockSize(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzAllocate(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzRestart(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzAbort(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzGetLastWrite(&Input, SystemTable, ImageHandle);
                break;
        }
    }
