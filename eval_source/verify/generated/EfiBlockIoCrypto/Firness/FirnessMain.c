    ...
    for (Step = 0; Step < Steps; Step++) {
        ...
        switch(DriverChoice%7)
        {
            case 0:
                ...
                break;
            case 1:
                Status = FuzzGetCapabilities(&Input, SystemTable, ImageHandle);
                break;
            case 2:
                Status = FuzzSetConfiguration(&Input, SystemTable, ImageHandle);
                break;
            case 3:
                Status = FuzzGetConfiguration(&Input, SystemTable, ImageHandle);
                break;
            case 4:
                Status = FuzzReadExtended(&Input, SystemTable, ImageHandle);
                break;
            case 5:
                Status = FuzzWriteExtended(&Input, SystemTable, ImageHandle);
                break;
            case 6:
                Status = FuzzFlushBlocks(&Input, SystemTable, ImageHandle);
                break;
        }
    }
