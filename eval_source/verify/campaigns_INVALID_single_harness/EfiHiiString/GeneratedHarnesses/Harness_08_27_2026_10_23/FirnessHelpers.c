  ...
  )
{
  ...
  // Determine the actual number of bytes to extract. Both operands are UINTN,
  // so this must not be written as "inputBuffer->Length - numBytes >= 0":
  // that comparison is unsigned and therefore always true, which let a short
  // buffer be over-read.
  ...
  // (and matching ReadByte() in FirnessHelpers_std.h).
  ...
  // Update the input buffer to remove the extracted bytes. This advances the
  // cursor *inside* the buffer -- advancing inputBuffer itself walks the
  // INPUT_BUFFER struct pointer off the end of its own object and leaves the
  // read cursor pinned at byte 0 forever.
